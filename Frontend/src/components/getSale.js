...
const GetSale = () => {
  ...
  const handleSubmitDate = (event) => {
    ...
    fetch(`http://localhost:5000/api/sales/getTotalSalesByDay/${date}`)
      .then((response) => {
        if (!response.ok) {
          throw new Error("No sales data found for the selected date.");
        }
  ...
  const handleSubmitMonth = (event) => {
    ...
    fetch(
      `http://localhost:5000/api/sales/getTotalSalesByMonth/${year}/${month}`
    )
      .then((response) => {
        if (!response.ok) {
          throw new Error("No sales data found for the selected month.");
        }
        return response.json();
      })
      .then((data) => {
  ...
  const handleSubmitYear = (event) => {
    ...
    fetch(`http://localhost:5000/api/sales/getTotalSalesByYear/${year}`)
      .then((response) => {
        if (!response.ok) {
          throw new Error("No sales data found for the selected year.");
        }
        return response.json();
      })
      .then((data) => {
  ...
  return (
    <div>
      ...
      <section className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-md mt-10">
        ...
        {dailySalesData && <div>{renderSalesTable(dailySalesData)}</div>}
      </section>

      <section className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-md mt-10">
        <h2 className="text-2xl font-bold mb-6 text-gray-800">
          Get Sale for a Month
        </h2>
        ...
        {monthlySalesData && <div>{renderSalesTable(monthlySalesData)}</div>}
      </section>
      ...
      <section className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-md mt-10">
        ...
        {yearlySalesData && <div>{renderSalesTable(yearlySalesData)}</div>}
      </section>
