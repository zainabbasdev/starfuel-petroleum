        ...
        <tbody>
          {data.map((sale, index) => (
            <tr key={index}>
              ...
              <td className="py-2 px-4 border-b text-center">{sale.balance}</td>
            </tr>
          ))}
        </tbody>
      </table>
  ...
  return (
    <div>
      ...
      <section className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-md mt-10">
        <h2 className="text-2xl font-bold mb-6 text-gray-800">Khata Book</h2>
        <div id="previousDaySalesData">{displayKhataData(khataData)}</div>
      </section>
      ...
      <section className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-md mt-10">
        <div className="flex justify-center w-full">
          <button
            ...
            onClick={() => (window.location.href = "/khata/addKhata")}
          >
            Add Khata
          </button>
          <button
            ...
            onClick={() => (window.location.href = "/khata/updateKhata")}
          >
            Update Khata
          </button>
          <button
            type="button"
            className="w-full ml-3 flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-yellow-600 hover:bg-yellow-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-yellow-500"
            onClick={() => (window.location.href = "/khata/getKhata")}
          >
            Get Khata
          </button>
        </div>
