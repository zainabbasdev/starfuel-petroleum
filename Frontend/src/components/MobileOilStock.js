...
const MobileOilStock = () => {
  const [salesData, setSalesData] = useState([]);

  useEffect(() => {
    fetch("http://localhost:5000/api/mobileOilStock/get", {
      method: "GET",
      headers: {
    ...
    return (
      <table className="min-w-full bg-white">
        ...
        <tbody>
          {data.map((sale) => (
            <tr key={sale._id}>
              <td className="py-2 px-4 border-b text-center">{sale.oilType}</td>
              <td className="py-2 px-4 border-b text-center">
                {sale.litersAdded.$numberDecimal}
              </td>
              <td className="py-2 px-4 border-b text-center">
                {sale.pricePerLiter.$numberDecimal}
              </td>
            </tr>
          ))}
        </tbody>
    ...
    <div>
      ...
      <section className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-md mt-10">
        ...
        <div id="currentMonthSalesData">
          {displayCurrentMonthSales(salesData)}
        </div>
      </section>
      ...
      <section className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-md mt-10">
        <div className="flex justify-center w-full">
          <Link
            to="/mobile-oil/mobileOilStock/addMobileOilStock"
