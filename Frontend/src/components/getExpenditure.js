...
const GetExpenditure = () => {
  ...
  const [dailyExpenditureData, setDailyExpenditureData] = useState(null);
  const [monthlyExpenditureData, setMonthlyExpenditureData] = useState(null);
  const [yearlyExpenditureData, setYearlyExpenditureData] = useState(null);
  ...
  const handleSubmitDate = (event) => {
    ...
    fetch(`http://localhost:5000/api/expenditure/getByDate/${date}`, {
      method: "GET",
      headers: {
      ...
      .then((response) => {
        if (!response.ok) {
          throw new Error("No Expenditure data found for the selected date.");
        }
        return response.json();
      })
      .then((data) => {
        setDailyExpenditureData(data);
      })
      .catch((error) => {
  ...
  const handleSubmitMonth = (event) => {
    ...
    if (month && year) {
      ...
      if (
        ...
        toast.error(
          "You can only get Expenditure for the current month or any previous month of the current year."
        );
        return;
      }
    }

    fetch(`http://localhost:5000/api/expenditure/getByMonth/${year}/${month}`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
      },
      ...
      .then((response) => {
        if (!response.ok) {
          throw new Error("No Expenditure data found for the selected month.");
        }
        return response.json();
      })
      .then((data) => {
  ...
  const handleSubmitYear = (event) => {
    ...
    fetch(`http://localhost:5000/api/expenditure/get-total/year/${year}`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
      },
      ...
      .then((response) => {
        if (!response.ok) {
          throw new Error("No Expenditure data found for the selected year.");
        }
        return response.json();
      })
      .then((data) => {
        setYearlyExpenditureData(data);
      })
      .catch((error) => {
        toast.error(error.message);
      });
  ...
  const displayExpenditureData = (data) => {
    if (!Array.isArray(data) || data.length === 0) {
      return <p className="text-gray-600">No Expenditure data available.</p>;
    }
    let totalExpenditure = 0;
    const rows = data.map((item) => {
      totalExpenditure += parseFloat(item.amount.$numberDecimal);
      return (
        <tr key={item._id}>
    ...
    return (
      <table className="min-w-full bg-white">
        ...
        <tbody>
          {rows}
          <tr>
            <td className="py-2 px-4 border-b text-center">Grand Total</td>
            <td className="py-2 px-4 border-b text-center">
              {totalExpenditure}
            </td>
          </tr>
  ...
  return (
    <div>
      ...
      <section className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-md mt-10">
        ...
        {dailyExpenditureData && (
          <div>{displayExpenditureData(dailyExpenditureData)}</div>
        )}
      </section>
      ...
      <section className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-md mt-10">
        ...
        {monthlyExpenditureData && (
          <div>{displayExpenditureData(monthlyExpenditureData)}</div>
        )}
      </section>

      <section className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-md mt-10">
        <h2 className="text-2xl font-bold mb-6 text-gray-800">
          Get Total Expenditure for a Year
        </h2>
        <form
        ...
        {yearlyExpenditureData && (
          <div>{displayYearlyExpenditureData(yearlyExpenditureData)}</div>
        )}
      </section>
